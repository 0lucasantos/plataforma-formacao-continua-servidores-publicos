...
export async function getQuizData(courseId: string): Promise<any> {
...
export async function submitQuiz(courseId: string, data: { attempt_id?: string; answers: number[] }): Promise<QuizResult> {
  return api(`/api/quiz/${courseId}`, {
    method: 'POST',
